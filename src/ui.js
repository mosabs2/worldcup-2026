...
(function () {
  ...
  function renderLeague(root) {
    ...
    const rows = entries.map(e => {
      const r = E.scoreEntry(e, resolved, sc);
      const hits = r.detail.filter(d => d.pts > 0).length;
      return { e, pts: r.pts, hits };
    }).sort((a, b) => b.pts - a.pts || (SIM.teams[b.e.c] ? SIM.teams[b.e.c].champ : 0) - (SIM.teams[a.e.c] ? SIM.teams[a.e.c].champ : 0));
    root.append(el('div', { class: 'card' },
      el('h3', null, 'Leaderboard', el('span', { class: 'right' }, Object.keys(resolved.groupWinners).length + ' of 12 groups resolved')),
      el('table', null,
        el('tr', null, ['#', 'Name', 'Champion pick', 'Model gives it', 'Correct calls', 'Points'].map((h, i) => el('th', { class: i > 2 ? 'num' : '' }, h))),
        rows.map((r, i) => el('tr', null,
          el('td', null, i + 1),
          el('td', null, el('b', null, r.e.n), r.e.src === 'local' ? el('span', { class: 'tiny' }, ' (local)') : ''),
          el('td', null, T[r.e.c] ? T[r.e.c].flag + ' ' + T[r.e.c].name : '—'),
          el('td', { class: 'num' }, T[r.e.c] ? pct(SIM.teams[r.e.c].champ) : '—'),
