...
for d, ms in by_date.items():
    ...
    for m in ms:
        ...
        # Guard against a 200-but-empty/not-yet-built summary: if the recorded scoreline
        # shows goals were scored but the summary yielded none, treat it as a bad fetch
        # and retry next cycle, rather than writing [] and freezing it espn-done forever
        # (which would silently drop those scorers from the Golden Boot board). A genuine
        # 0-0 has score sum 0, so it still records [] correctly.
        if not goals and (m["score"]["team1"] + m["score"]["team2"]) > 0:
            continue
        m["goals"] = goals          # [] only for a genuine goalless match
        m["goals_src"] = "espn"
        n += 1
